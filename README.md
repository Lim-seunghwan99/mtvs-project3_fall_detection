# 🌼 **장애인, 노인, 경계선지능인의 생활 개선 및 복지 향상을 위한 낙상 인식 및 대응 시스템** - '피우다' 프로젝트
...
### 서비스 설명
10개의 프레임을 javascript의 mediapipe 라이브러리를 통해 실시간으로 관절 좌표를 추출합니다.
각 33가지의 관절 좌표는 x, y, z 좌표로 이루어져 있으며, 각 관절의 x, y, z 좌표를 10개의 프레임에 대해 3차원 배열로 저장합니다.
총 990개의 관절 좌표중 결측치가 있으면 3차 스플라인 보간법을 통해 결측치를 보간합니다.
이후 990개의 관절 좌표를 1차원으로 flatten하여 LSTM 모델에 입력합니다.
모델에서 예측한 결과가 낙상으로 판단되면, solapi API를 통해 보호자에게 SMS를 전송하며, 낙상과정 이미지 하나를 서버에 저장하고, DB에 파일이름을 저장합니다.
---
